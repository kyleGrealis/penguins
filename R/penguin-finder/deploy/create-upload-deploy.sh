...
if [ -z "${CONNECT_SERVER}" ] ; then
    ...
    echo 
    echo "    export CONNECT_SERVER='https://positconnect.med.miami.edu/connect/'"
    exit 1
fi
...
if [ -z "${CONNECT_API_KEY}" ] ; then
    ...
    echo
    echo "    export CONNECT_API_KEY='ycgq8rtFRxwZBheMq4O2aXAEwisOZFDe'"
    exit 1
fi
...
tar czf "${BUNDLE_PATH}" manifest.json app.R www
